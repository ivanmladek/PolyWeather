...
| 第 6–8 周 | 状态/缓存统一 + 可观测性 | `daily_records/open_meteo_cache` 主读 SQLite；离线脚本也切到 SQLite 优先；指标（请求量/429/延迟/命中率）；Prometheus/Alertmanager/Grafana 最小链路与告警阈值 | 可先用 SQLite/Redis，后续再上 Postgres |
